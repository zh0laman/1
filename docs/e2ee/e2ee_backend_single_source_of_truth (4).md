...
- current canonical backup нужно трактовать как deterministic identity-restore contract
- legacy rows с backfill/inference нужно трактовать как compatibility contract, а не как доказанный original crypto contract
...
- отдавать deterministic backup contract для current canonical rows и явный compatibility contract для inferred legacy rows
