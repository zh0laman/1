...
4. Если backup есть и пользователь знает master password:
   ...
   - если decrypt не прошел, не отправляем guessed `public_key`, а безопасно переключаемся на `fresh-key flow` или `QR linked-device flow`
5. Если backup нет или пароль неизвестен:
   - регистрируем fresh web key
   - если пользователь заранее ввел новый master password, после успешного `device/register` можно сразу сохранить новый canonical recovery backup через `POST /api/v1/e2ee/backup`
   - старую историю потом открываем через QR pairing или не открываем вовсе

Рекомендуемый UX:

- по умолчанию ordinary login должен идти как `fresh-key flow`
- `restore-first flow` стоит включать только если на аккаунте уже ожидается recovery backup и пользователю действительно нужна старая история без mobile app
- если `GET /e2ee/backup` вернул `204`, но пользователь уже ввел новый master password, frontend может завершить fresh-key onboarding и тут же сохранить новый canonical recovery backup для следующих browser slots
...
`GET /api/v1/e2ee/backup` возвращает encrypted blob плюс explicit metadata.

Для current canonical backup этот response можно считать deterministic restore contract.

Для legacy rows, где metadata была восстановлена через backfill/normalization/inference, этот response нужно считать compatibility contract, а не доказанным historical crypto proof.
...
- если такой legacy backup не расшифровывается по backend-declared contract, не нужно добавлять guessing fallback; safe path это `fresh web key` и при необходимости `QR + history sync`
...
- QR-local state нельзя переиспользовать между password flow и QR flow
- при переключении на ordinary password login, logout или account switch нужно очищать локальный `pairing_id`, `session_token`, `qr_payload` и связанные progress-флаги
...
10. Если `backup_key_format = raw-32-bytes`, трактует plaintext как raw `32-byte X25519 private key` и нормализует его в importable representation
11. Из private key детерминированно вычисляет `public_key`

Legacy fallback:

- если `backup_key_format = raw-32-bytes`, шаг 8 меняется
- после decrypt frontend получает raw `32-byte` private key
- этот plaintext нужно импортировать как raw `X25519 private key`, а не как `pkcs8`
...
- `has_backup = false` в ordinary `/e2ee/device/register` response не всегда означает, что backup физически отсутствует в БД
- для legacy compatibility users backend может специально не рекламировать старый compatibility backup как deterministic ordinary restore path
- в этом случае frontend должен считать password bootstrap завершенным, продолжать как `fresh-key flow` и при необходимости предлагать отдельный новый `QR linked-device flow` для old history
...
- stale QR-local state очищается при выходе из QR flow и перед password login
