...
- `restored identity key from backup` может дать доступ и к старой E2EE истории уже после обычного login, без mobile app
...
`GET /api/v1/e2ee/backup` возвращает encrypted blob плюс deterministic metadata.
...
8. Получает plaintext формата `pkcs8-der-base64`
9. Импортирует private key как `X25519`
10. Из private key детерминированно вычисляет `public_key`
...
- Restore flow делает local decrypt и import `pkcs8-der-base64`
