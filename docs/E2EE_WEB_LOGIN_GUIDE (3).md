...
2. через `salt + kdf_algorithm + kdf_params` выводит key-encryption key
...
- frontend не должен угадывать `iterations`, `hash`, `memory`, `parallelism`
- всё это нужно брать из `backup.kdf_params`
- для legacy `pbkdf2` / `pbkdf2-sha256` version `1` backend теперь возвращает backfilled contract:
  - `iterations = 310000`
  - `hash = sha256`
  - `key_length = 32`
- если `kdf_params` отсутствует и backend не смог его вывести, frontend должен считать такой backup `unsupported_legacy_backup`, а не подбирать параметры наугад
