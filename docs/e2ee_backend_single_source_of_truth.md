...
- backup для current canonical rows нужно трактовать как deterministic identity-restore contract, а для inferred legacy rows как compatibility contract
...
Для current canonical backup этот response нужно считать deterministic contract.
...
### Шаг 3. Web регистрирует себя как отдельное устройство
...
- "если у тебя есть backup, ты сразу увидишь всю старую историю"
...
- не переиспользовать guessed или unsupported private key material
...
- "если restore-first flow не сработал, старая история будет подключена после подтверждения на мобильном устройстве"
...
- `GET /api/v1/e2ee/backup` отдает полный deterministic contract для current canonical backup и явный compatibility contract для inferred legacy rows
...
3. Password login поддерживает `fresh web key` и `restored identity key from backup`.
4. Backup помогает recovery и может открыть old history без QR, если restored identity совпадает, но legacy inferred rows не являются криптографически доказанным restore contract.
5. Если вход идет с fresh web key, старая история для нового web открывается через `QR + history sync / re-wrap`.
